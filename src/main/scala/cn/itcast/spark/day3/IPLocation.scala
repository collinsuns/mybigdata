...
object IPLocation {
  ...
  val data2MySQL = (iterator: Iterator[(String, Int)]) => {
    ...
    try {
      conn = DriverManager.getConnection("jdbc:mysql://hd1:3306/bigdata", "root", "root")
  ...
  def main(args: Array[String]) {
    ...
    val ipRulesRdd = sc.textFile("C://test//IPLocation//ip.txt").map(line =>{
    ...
    //加载要处理的数据
    val ipsRDD = sc.textFile("C://test//IPLocation//20090121000132.394251.http.format").map(line => {
      val fields = line.split("\\|")
      fields(1)
    })
